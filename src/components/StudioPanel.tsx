  ...
  return (
    <section className="workspace-panel studio-panel" aria-labelledby="studio-heading">
      <header className="panel-header">
        ...
        <button className="icon-button desktop-only" type="button" onClick={onCollapse} aria-label="Collapse studio panel">
      ...
      <div className="panel-scroll studio-scroll">
          ...
          {artifactDefinitions.map((definition) => (
            <div className={`studio-tool tint-${definition.tint}`} key={definition.type}>
              <button
                ...
                type="button"
                onClick={() => sourceCount > 0 ? onGenerate(definition.type, defaultArtifactConfig(definition.type, settings.outputLanguage)) : onAddSource()}
              >
                <ArtifactIcon type={definition.type} size={19} />
                <strong>{definition.label}</strong>
              </button>
              {definition.editable && (
                <button className="studio-edit-button" type="button" onClick={() => onCustomize(definition.type)} aria-label={`Customize ${definition.label}`}>
                  <PenLine size={16} />
                </button>
              )}
            </div>
        ...
        {artifacts.length > 0 && (
          <section className="studio-section" aria-labelledby="outputs-title">
            <div className="studio-section-heading"><h3 id="outputs-title">Studio outputs</h3><span>{artifacts.length}</span></div>
            <div className="studio-output-list">
              {artifacts.slice().sort((a, b) => b.createdAt - a.createdAt).map((artifact) => {
                const definition = artifactDefinitions.find((item) => item.type === artifact.type)
                return (
                  <article className="studio-output" key={artifact.id}>
                    <button className="studio-output-main" type="button" onClick={() => artifact.status === 'ready' && onOpenArtifact(artifact)} title={artifact.error}>
                      <span className={`output-icon tint-${definition?.tint ?? 'violet'}`}>
                        {artifact.status === 'generating' ? <span className="spinner" /> : artifact.status === 'error' ? <XCircle size={18} /> : <ArtifactIcon type={artifact.type} size={18} />}
                      </span>
        ...
        <section className="studio-section notes-section" aria-labelledby="notes-title">
          <div className="studio-section-heading"><h3 id="notes-title">Notes</h3><span>{notes.length}</span></div>
          {notes.length === 0 ? (
            ...
            <div className="note-list">
              {notes.slice().sort((a, b) => b.createdAt - a.createdAt).map((note) => (
