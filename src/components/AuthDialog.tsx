...
export function AuthDialog({
  ...
  if (!account.isAnonymous) {
    return (
      <Modal
        ...
        onClose={close}
        title="Your account"
        description="Manage the permanent account protecting this workspace."
      ...
      >
        <div className="account-identity-card">
          ...
          <div>
            <strong>{account.email || 'Permanent account'}</strong>
            <small>Notebooks sync to this Supabase identity</small>
          </div>
          <em>Protected</em>
        </div>
        {prompt && (
          <div className="auth-callout success">
            ...
            <div>
              <strong>{prompt === 'confirmed' ? 'Email confirmed' : 'Recovery link accepted'}</strong>
              <span>
                {prompt === 'confirmed'
                  ? 'Choose a password so you can sign in again on any device.'
        ...
        <div className="account-signout-row">
          <div>
            ...
            <span>A fresh guest workspace will open afterward.</span>
          </div>
          <button
  ...
  return (
    <Modal
      ...
      description={
        guestMode === 'reset'
          ? 'We email you a link to choose a new password. No password needed on this step.'
          : 'Connect your NotebookLM workspace to a permanent Supabase account.'
      }
      className="auth-modal"
    >
      {prompt && (
        <div className="auth-callout warning" role="alert">
          ...
          <div>
            <strong>Account link could not be applied</strong>
            <span>This link may be expired or already used. Request a new confirmation or password reset email.</span>
          </div>
        </div>
      )}
      {guestMode !== 'reset' && (
        <div className="auth-mode-switch" role="tablist" aria-label="Account action">
          <button
            type="button"
            role="tab"
            aria-selected={guestMode === 'upgrade'}
            className={guestMode === 'upgrade' ? 'active' : ''}
            onClick={() => {
              setGuestMode('upgrade')
              resetFeedback()
            }}
          ...
          <button
            type="button"
            role="tab"
            aria-selected={guestMode === 'sign-in'}
            className={guestMode === 'sign-in' ? 'active' : ''}
            onClick={() => {
      ...
      {guestMode === 'reset' ? (
        <>
          ...
          <div className="auth-copy">
            <h3>Send a recovery link</h3>
            <p>Enter the email address of your account. The link in that email lets you set a new password.</p>
          </div>
        ...
        <>
          <div className="workspace-continuity">
            <span
              className="continuity-count"
              aria-label={`${notebookCount} guest notebook${notebookCount === 1 ? '' : 's'}`}
            >
              <strong>{notebookCount}</strong>
              <small>guest notebook{notebookCount === 1 ? '' : 's'}</small>
            </span>
            ...
            <span className="continuity-account">
              <Mail size={20} />
              <small>your account</small>
            </span>
          </div>
          <div className="auth-copy">
            <h3>Keep the same workspace</h3>
            <p>We attach a verified email to this guest identity, so your notebooks stay exactly where they are.</p>
          </div>
        ...
        <>
          {notebookCount > 0 && (
            <div className="auth-callout warning">
              <TriangleAlert size={19} />
              <div>
                <strong>Guest notebooks stay behind</strong>
                <span>
                  Signing in switches this browser to another workspace. These {notebookCount} notebook
                  {notebookCount === 1 ? '' : 's'} will not be moved.
                </span>
              </div>
            </div>
          )}
          <form className="auth-form" onSubmit={submitSignIn}>
            ...
            {notebookCount > 0 && (
              <label className="auth-confirm">
                ...
                />
                <span>I understand this switches away from the current guest notebooks.</span>
              </label>
            )}
      ...
      <div className="guest-note">
        <ShieldCheck size={16} />
        <span>
          Guest access remains available. Create an account only when you want this workspace on another device.
        </span>
      </div>
    </Modal>
  )
