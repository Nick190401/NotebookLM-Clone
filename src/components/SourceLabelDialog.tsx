...
export function SourceLabelDialog({
  ...
  return (
    ...
    >
      <form
        className="source-label-form"
        onSubmit={(event) => {
          event.preventDefault()
          submit()
        }}
      >
        <label className="field-label" htmlFor="source-label-name">
          Label name
        </label>
        ...
        {options.length > 0 && (
          <div className="source-label-suggestions">
            <span>Existing labels</span>
            <div>
              {options.map((label) => (
                <button type="button" key={label} onClick={() => setDraft(label)}>
                  {label}
                </button>
              ))}
            </div>
        ...
        <div className="modal-actions source-label-actions">
          {currentLabel && (
            <button className="text-button source-label-remove" type="button" onClick={onRemove}>
