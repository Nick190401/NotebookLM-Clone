...
import { BrainCircuit, CirclePlay, ExternalLink, FileSearch, FileText, Globe2, HardDrive, Link2, LoaderCircle, Search, Sparkles, Type, UploadCloud, Zap } from 'lucide-react'
import { discoverSources, importSourceUrl, runDeepResearch, uploadSource, type DeepResearchResult, type DiscoveredSource } from '../lib/api'
...
export function AddSourceDialog({ open, language, initialQuery = '', initialResearchMode = 'fast', onClose, onAdd }: AddSourceDialogProps) {
        ...
        {tab === 'discover' && (
          <div className="discover-view">
            <div className="research-mode-grid" role="group" aria-label="Research mode">
              <button className={researchMode === 'fast' ? 'selected' : ''} type="button" aria-pressed={researchMode === 'fast'} disabled={busy} onClick={() => chooseResearchMode('fast')}><span><Zap size={17} /></span><strong>Fast Research</strong><small>One focused web scan</small></button>
              <button className={researchMode === 'deep' ? 'selected' : ''} type="button" aria-pressed={researchMode === 'deep'} disabled={busy} onClick={() => chooseResearchMode('deep')}><span><BrainCircuit size={17} /></span><strong>Deep Research</strong><small>Multi-step investigation + report</small></button>
            </div>
            <div className="discover-search"><Search size={20} /><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); runDiscovery() } }} placeholder={researchMode === 'deep' ? 'Ask a detailed research question' : 'Research a topic or ask a question'} autoFocus /><button className="primary-button compact" type="button" disabled={busy} onClick={runDiscovery}>{busy ? (researchMode === 'deep' ? 'Researching…' : 'Searching…') : (researchMode === 'deep' ? 'Start research' : 'Search')}</button></div>
            <div className="research-capabilities"><span><Globe2 size={14} /> Live web</span>{researchMode === 'deep' && <><span><Search size={14} /> Multiple searches</span><span><FileSearch size={14} /> Visits sources</span></>}</div>
            {discoverResults.length === 0 ? (busy && researchMode === 'deep' ? <div className="deep-research-progress"><span className="research-orbit"><LoaderCircle className="spin" size={24} /></span><h3>Investigating the web</h3><p>Groq is browsing from multiple angles, cross-checking visited sources, and synthesizing its findings.</p><div aria-hidden="true"><i /><i /><i /></div><small>This can take up to two minutes.</small></div> : <div className="discover-empty">{busy ? <LoaderCircle className="spin" size={35} /> : researchMode === 'deep' ? <BrainCircuit size={35} /> : <Search size={35} />}<h3>{researchMode === 'deep' ? 'Build a research dossier' : 'Find credible sources'}</h3><p>{researchMode === 'deep' ? 'Ask a focused question to create a detailed report and a reviewable source set.' : 'AI-assisted live web results can be reviewed before their full content is imported.'}</p></div>) : (
              <div className="discover-results">
                {deepResearch && <article className="research-report-preview"><header><span><Sparkles size={18} /></span><div><small>Deep Research report</small><strong>{query.trim()}</strong></div><em>{deepResearch.toolCount} research actions</em></header><div>{renderResearchReport(deepResearch.report)}</div></article>}
                <div className="result-heading"><strong>{deepResearch ? 'Sources reviewed' : 'Suggested sources'}</strong><span>{selectedResults.size} selected</span></div>
                {discoverResults.map((source) => <div className="discover-result" key={source.id}><input type="checkbox" aria-label={`Select ${source.title}`} checked={selectedResults.has(source.id)} onChange={() => setSelectedResults((current) => { const next = new Set(current); if (next.has(source.id)) next.delete(source.id); else next.add(source.id); return next })} /><span><strong>{source.title}</strong><small>{source.summary}</small><span className="discover-result-meta"><em>{source.url}</em><a href={source.url} target="_blank" rel="noreferrer">Open <ExternalLink size={12} /></a></span></span></div>)}
                <div className="modal-actions"><button className="secondary-button" type="button" disabled={busy} onClick={() => { setDiscoverResults([]); setSelectedResults(new Set()); setDeepResearch(null) }}>New research</button><button className="primary-button" type="button" disabled={busy || (!deepResearch && selectedResults.size === 0)} onClick={importDiscovery}>{busy ? 'Importing…' : researchImportLabel}</button></div>
              </div>
            )}
          </div>
        )}
