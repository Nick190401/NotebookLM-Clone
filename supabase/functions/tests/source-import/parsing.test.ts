...
Deno.test('HTML extraction keeps article text and removes executable or navigation content', () => {
  ...
  assert(!result.content.includes('steal') && !result.content.includes('Menu item') && !result.content.includes('Footer'), 'page chrome leaked into source text')
})

Deno.test('DOCX extraction reads paragraph text without Node-only document libraries', () => {
  const archive = zipSync({
    '[Content_Types].xml': strToU8('<Types/>'),
    'word/document.xml': strToU8(`<?xml version="1.0"?><w:document xmlns:w="urn:test"><w:body><w:p><w:r><w:t>First paragraph</w:t></w:r></w:p><w:p><w:r><w:t>Second &amp; grounded</w:t></w:r></w:p></w:body></w:document>`),
  })
