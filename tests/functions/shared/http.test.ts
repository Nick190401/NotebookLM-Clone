...
function withEnvironment<T>(run: () => Promise<T>) {
  ...
  return run().finally(() => {
    ...
    if (previousKey === undefined) Deno.env.delete('SUPABASE_PUBLISHABLE_KEY')
    else Deno.env.set('SUPABASE_PUBLISHABLE_KEY', previousKey)
    globalThis.fetch = previousFetch
  })
}

Deno.test('authenticated context validates the bearer session with Supabase Auth', () =>
  withEnvironment(async () => {
    ...
    const context = await authenticatedContext(
      new Request('https://function.test', { headers: { Authorization: 'Bearer user-jwt' } }),
    )
...
Deno.test('authenticated context rejects an invalid session', () =>
  withEnvironment(async () => {
    ...
    assert(
      caught instanceof HttpError && caught.code === 'AUTH_INVALID' && caught.status === 401,
      'invalid session was not rejected',
    )
  }),
)

Deno.test('Supabase RPC keeps the caller JWT so RLS remains active', () =>
  withEnvironment(async () => {
    let received: Request | undefined
    globalThis.fetch = async (input, init) => {
      received = new Request(input, init)
    ...
    const result = await supabaseRpc<{ id: string }[]>(
      ...
      'load_ai_sources',
      { requested_notebook_id: 'notebook-a', requested_source_ids: ['source-a'] },
    )
