...
describe('source organization', () => {
  it('suggests readable topic labels while preserving manual labels', () => {
    const unlabeled = { ...makeSource({ title: 'Rail plan', kind: 'text', origin: 'Test', content: 'public-transport public-transport public-transport' }), topics: ['public-transport'] }
    const manual = { ...makeSource({ title: 'Budget', kind: 'pdf', origin: 'Test', content: 'finance finance finance' }), label: 'My research' }

    expect(suggestedSourceLabel(unlabeled)).toBe('Public Transport')
    expect(organizeSources([unlabeled, manual])).toEqual([
      { ...unlabeled, label: 'Public Transport' },
      manual,
    ])
  })
